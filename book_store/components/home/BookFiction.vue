  ...
  <section class="mx-auto mt-12 w-full max-w-[1500px] px-4">
    ...
    <div class="mb-8 flex items-center justify-between">
      <div class="flex items-center">
        ...
        >
          <BookOpenText class="h-8 w-8 text-darkgreen dark:text-foreground" />
        </div>
        <div>
          <h2
          ...
          <p class="mb-0 text-sm text-muted-foreground">
            Discover compelling stories and thought-provoking narratives
          </p>
        </div>
      </div>
    ...
    <div v-if="fictionBooks.length === 0">
      <Swiper
        :slides-per-view="2"
        :space-between="16"
        :loop="true"
    ...
    <div v-else class="relative">
      ...
      <Swiper
        ...
        :loop="true"
        :breakpoints="swiperBreakpoints"
        class="fiction-swiper px-1 pb-[50px]"
        @swiper="onSwiper"
      >
        <SwiperSlide v-for="(book, i) in fictionBooks" :key="i">
          <div
          ...
          >
            ...
            <div class="relative flex items-center justify-center p-5">
              <div
                class="aspect-[2/3] w-[80%] max-w-[180px] overflow-hidden bg-muted transition-all duration-[400ms] ease-in-out group-hover:-translate-y-2 group-hover:scale-105"
              >
                <img
                  :src="book?.cover_url"
                  :alt="book.title"
              ...
              <!-- Stock Badge -->
              <span
                class="absolute bottom-3 left-1/2 -translate-x-1/2 whitespace-nowrap rounded-full bg-customyellow px-2 py-0.5 text-[10px] font-semibold tracking-[0.5px] text-customblack"
              >
                Literary Fiction
              </span>
            </div>
...
const fictionBooks = computed<Book[]>(
  () => homeSubjects.value["literary fiction"] || []
);
...
.fiction-swiper :deep(.swiper-pagination-bullet) {
  background: #435058;
  opacity: 0.3;
}

.fiction-swiper :deep(.swiper-pagination-bullet-active) {
  opacity: 1;
  background: #435058;
}
</style>
